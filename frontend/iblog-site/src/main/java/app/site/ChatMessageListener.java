...
public class ChatMessageListener extends WSContext implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        Optional<WSChatMessage> chatMessageOptional = CHAT_MESSAGE_CACHE.findById(messageId);
        if (chatMessageOptional.isEmpty()) return;

        WSChatMessage chatMessage = chatMessageOptional.get();
        ...
        for (String id : groupUserIds) {
            if (chatMessage.type == WSChatMessageType.USER_JOIN) {
                if (id.equals(String.valueOf(chatMessage.userJoinMessage.chatMember.userId))) {
                    continue;
                }
            } else if (chatMessage.type == WSChatMessageType.CHAT) {
                if (id.equals(String.valueOf(chatMessage.chatContentMessage.from.userId))) {
                    continue;
                }
            } else {
                return;
